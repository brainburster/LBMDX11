...
LBM_Multicomponent::LBM_Multicomponent(decltype(dx11_wnd) wnd)
	: dx11_wnd{ wnd },
	last_control_point{
		{-1.f, -1.f},
		{-1.f, -1.f},
	},
	display_setting{ 0, 0, 0, 0 }, pause{ false }
{
}
...
void LBM_Multicomponent::init_shaders()
{
	...
	hr = D3DReadFileToBlob(L"shaders/cs_lbm_init.cso",
		blob.ReleaseAndGetAddressOf());
	assert(SUCCEEDED(hr));
	hr = device->CreateComputeShader(blob->GetBufferPointer(),
		blob->GetBufferSize(), nullptr,
		cs_lbm_init.GetAddressOf());
	assert(SUCCEEDED(hr));

	hr = D3DReadFileToBlob(L"shaders/cs_lbm_collision.cso",
		blob.ReleaseAndGetAddressOf());
	assert(SUCCEEDED(hr));
	hr = device->CreateComputeShader(blob->GetBufferPointer(),
		blob->GetBufferSize(), nullptr,
	...
	hr = D3DReadFileToBlob(L"shaders/cs_lbm_streaming.cso",
		blob.ReleaseAndGetAddressOf());
	assert(SUCCEEDED(hr));
	hr = device->CreateComputeShader(blob->GetBufferPointer(),
		blob->GetBufferSize(), nullptr,
	...
	hr = D3DReadFileToBlob(L"shaders/cs_lbm_visualization.cso",
		blob.ReleaseAndGetAddressOf());
	assert(SUCCEEDED(hr));
	hr = device->CreateComputeShader(blob->GetBufferPointer(),
		blob->GetBufferSize(), nullptr,
		cs_lbm_visualization.GetAddressOf());
	assert(SUCCEEDED(hr));

	hr = D3DReadFileToBlob(L"shaders/cs_lbm_force_calculation.cso",
		blob.ReleaseAndGetAddressOf());
	assert(SUCCEEDED(hr));
	hr = device->CreateComputeShader(blob->GetBufferPointer(),
		blob->GetBufferSize(), nullptr,
		cs_lbm_force_calculation.GetAddressOf());
	assert(SUCCEEDED(hr));

	hr = D3DReadFileToBlob(L"shaders/cs_lbm_moment_update.cso",
		blob.ReleaseAndGetAddressOf());
	assert(SUCCEEDED(hr));
	hr = device->CreateComputeShader(blob->GetBufferPointer(),
		blob->GetBufferSize(), nullptr,
		cs_lbm_moment_update.GetAddressOf());
	assert(SUCCEEDED(hr));
}
...
void LBM_Multicomponent::update_displaysetting()
{
	...
	HRESULT hr = ctx->Map(cbuf_display_setting.Get(), 0, D3D11_MAP_WRITE_DISCARD,
...
void LBM_Multicomponent::init()
{
	...
	ctx->Dispatch((dx11_wnd->getWidth() - 1) / 32 + 1,
		(dx11_wnd->getHeight() - 1) / 32 + 1, 1);
	fence();
}
...
#pragma warning(disable: 6031)
	while (dx11_wnd && !dx11_wnd->app_should_close())
	{
		dx11_wnd->PeekMsg();
		handleInput();
		update();
...
void LBM_Multicomponent::init_resources()
{
	...
	assert(SUCCEEDED(hr));
	hr = device->CreateUnorderedAccessView(tex_array_f_in[0].Get(), &uav_desc,
		uav_tex_array_f_in[0].GetAddressOf());
	hr = device->CreateUnorderedAccessView(tex_array_f_in[1].Get(), &uav_desc,
		uav_tex_array_f_in[1].GetAddressOf());
	hr = device->CreateUnorderedAccessView(tex_array_f_in[2].Get(), &uav_desc,
		uav_tex_array_f_in[2].GetAddressOf());
	hr = device->CreateUnorderedAccessView(tex_array_f_out[0].Get(), &uav_desc,
		uav_tex_array_f_out[0].GetAddressOf());
	hr = device->CreateUnorderedAccessView(tex_array_f_out[1].Get(), &uav_desc,
		uav_tex_array_f_out[1].GetAddressOf());
	hr = device->CreateUnorderedAccessView(tex_array_f_out[2].Get(), &uav_desc,
		uav_tex_array_f_out[2].GetAddressOf());
	assert(SUCCEEDED(hr));
	...
	SimSetting sim_setting = { dx11_wnd->getWidth(), dx11_wnd->getHeight(),
							  grid_size };
	InitData = {};
	InitData.pSysMem = &sim_setting;
	hr = device->CreateBuffer(&buf_desc, &InitData,
		cbuf_sim_setting.GetAddressOf());
	assert(SUCCEEDED(hr));
	...
	hr = device->CreateBuffer(&buf_desc, &InitData,
		cbuf_display_setting.GetAddressOf());
	assert(SUCCEEDED(hr));
}
...
void LBM_Multicomponent::bind_resources()
{
	...
	ctx->CSSetUnorderedAccessViews(3, 1, uav_tex_array_f_out[0].GetAddressOf(),
		0);
	ctx->CSSetUnorderedAccessViews(4, 1, uav_tex_array_f_out[1].GetAddressOf(),
		0);
	ctx->CSSetUnorderedAccessViews(5, 1, uav_tex_array_f_out[2].GetAddressOf(),
		0);
...
void LBM_Multicomponent::set_input_callback()
{
	const auto onmousemove = [&](WPARAM wparam, LPARAM lparam) {
		...
		if (wparam & MK_SHIFT)
		{
			add_control_point(pos, { brush_size * grid_size, 0.f });
		}
		else if (wparam & MK_LBUTTON)
		{
			add_control_point(pos, { brush_size * grid_size, 1.f });
		}
		else if (wparam & MK_RBUTTON)
		{
			add_control_point(pos, { brush_size * grid_size, 2.f });
		}
		else if (wparam & MK_MBUTTON)
		{
			add_control_point(pos, { brush_size * grid_size, 3.f });
		}
