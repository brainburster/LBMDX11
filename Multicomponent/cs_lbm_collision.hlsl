...
static const float k[3] = { 1.0f, 1.05f, 1.2f };
...
void main( uint3 DTid : SV_DispatchThreadID )
{
    ...
    float rho[3] = { 0.f, 0.f, 0.f };
    float2 F = { 0.f, 0.f };
    float2 u = { 0.f, 0.f };
    
    rho[0] = f_in[0][uint3(pos, 9)];
    rho[1] = f_in[1][uint3(pos, 9)];
    rho[2] = f_in[2][uint3(pos, 9)];
    
    if ((rho[0] + rho[1] + rho[2]) < 0 || uav_display[pos].w < 0)
    {
        for (i = 0; i < 9; i++)
        {
            f_out[0][uint3(pos, i)] = 0.f;
            f_out[1][uint3(pos, i)] = 0.f;
            f_out[2][uint3(pos, i)] = 0.f;
        }
        return;
    }
